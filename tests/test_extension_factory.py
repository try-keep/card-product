...
def test_make_past_due_payment_partial_coverage(factory):
    ...
    assert payment_result['total_amount'] == 1200.00
    assert payment_result['remaining_amount'] == pytest.approx(0, abs=1e-2)
    assert ext1.get_past_due_amount(check_date) == pytest.approx(0, abs=1e-2)
    assert ext2.get_past_due_amount(
        check_date) == pytest.approx(56.67, abs=1e-2)
...
def test_make_past_due_payment_double_first_extension(factory):
    ...
    assert len(payment_result['payments']) == 2
    assert payment_result['total_amount'] == 1060.00
...
def test_make_past_due_payment_no_past_due(factory):
    ...
    assert payment_result['total_amount'] == 500.00
    assert payment_result['remaining_amount'] == 0
...
def test_make_next_due_payment_before_due_date(factory):
    ...
    assert len(payment_result['payments']) == 1
    assert payment_result['total_amount'] == 530.00
    assert payment_result['remaining_amount'] == 0.0
...
def test_make_next_due_payment_multiple_extensions(factory):
    ...
    assert len(payment_result['payments']) == 2
    assert payment_result['total_amount'] == 1600.00
    assert payment_result['remaining_amount'] == pytest.approx(
        343.33, rel=1e-2)
